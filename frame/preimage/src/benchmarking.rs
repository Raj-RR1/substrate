...
benchmarks! {
	...
	note_requested_preimage {
		...
		whitelist_account!(caller);
		let (preimage, hash) = sized_preimage_and_hash::<T>(s);
		assert_ok!(Preimage::<T>::request_preimage(T::ManagerOrigin::successful_origin().expect("ManagerOrigin has no successful origin required for the benchmark"), hash.clone()));
	...
	note_no_deposit_preimage {
		...
		let (preimage, hash) = sized_preimage_and_hash::<T>(s);
		assert_ok!(Preimage::<T>::request_preimage(T::ManagerOrigin::successful_origin().expect("ManagerOrigin has no successful origin required for the benchmark"), hash.clone()));
	}: note_preimage<T::Origin>(T::ManagerOrigin::successful_origin().map_err(|_| BenchmarkError::Weightless)?, preimage)
	verify {
		assert!(Preimage::<T>::have_preimage(&hash));
	}
	...
	unnote_no_deposit_preimage {
		let (preimage, hash) = preimage_and_hash::<T>();
		assert_ok!(Preimage::<T>::note_preimage(T::ManagerOrigin::successful_origin().expect("ManagerOrigin has no successful origin required for the benchmark"), preimage));
	}: unnote_preimage<T::Origin>(T::ManagerOrigin::successful_origin().map_err(|_| BenchmarkError::Weightless)?, hash.clone())
	verify {
		assert!(!Preimage::<T>::have_preimage(&hash));
	}
	...
	}: _<T::Origin>(T::ManagerOrigin::successful_origin().map_err(|_| BenchmarkError::Weightless)?, hash)
	verify {
	...
	request_no_deposit_preimage {
		let (preimage, hash) = preimage_and_hash::<T>();
		assert_ok!(Preimage::<T>::note_preimage(T::ManagerOrigin::successful_origin().expect("ManagerOrigin has no successful origin required for the benchmark"), preimage));
	}: request_preimage<T::Origin>(T::ManagerOrigin::successful_origin().map_err(|_| BenchmarkError::Weightless)?, hash)
	verify {
		assert_eq!(StatusFor::<T>::get(&hash), Some(RequestStatus::Requested(1)));
	}
	...
	unrequest_preimage {
		...
		assert_ok!(Preimage::<T>::request_preimage(T::ManagerOrigin::successful_origin().expect("ManagerOrigin has no successful origin required for the benchmark"), hash.clone()));
		assert_ok!(Preimage::<T>::note_preimage(T::ManagerOrigin::successful_origin().expect("ManagerOrigin has no successful origin required for the benchmark"), preimage));
	}: _<T::Origin>(T::ManagerOrigin::successful_origin().map_err(|_| BenchmarkError::Weightless)?, hash.clone())
	verify {
	...
	unrequest_unnoted_preimage {
		let (_, hash) = preimage_and_hash::<T>();
		assert_ok!(Preimage::<T>::request_preimage(T::ManagerOrigin::successful_origin().expect("ManagerOrigin has no successful origin required for the benchmark"), hash.clone()));
	}: unrequest_preimage<T::Origin>(T::ManagerOrigin::successful_origin().map_err(|_| BenchmarkError::Weightless)?, hash.clone())
	verify {
		assert_eq!(StatusFor::<T>::get(&hash), None);
	}
